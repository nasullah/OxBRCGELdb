...
<head>
    ...
    <title>Aliquots in transit to MDC </title>
</head>
    ...
    <table class="table table-bordered margin-top-medium">
        ...
        <g:each in="${aliquotInstanceList}" status="i" var="aliquotInstance">
            <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                ...
                <td><a class='btn btn-primary btn-xs' <g:link controller="sampleTrackingEvent" action="create" params="['identifiedSample.id': aliquotInstance?.id, 'sampleTrackingEventType.id':SampleTrackingEventType.findBySampleTrackingEventTypeName('Received at MDC lab')?.id]"><i class="glyphicon glyphicon-send"></i> Received at MDC</g:link></td>
